...
if($conn)
{
   ...
   $sql1="select * from admin1 where username='$user' and password='$password2'";
   $studentname=mysqli_query($conn,$sql1);  
   $student1=mysqli_fetch_assoc($studentname);
   $user2=$student1['username'];
   $password3=$student1['password'];
   if($user2==$user && $password2==$password3)
 {
    
    echo "Congratulation! your login Succefully";
    //$_SESSION["check1"]=
 }
 else{

    $_SESSION["error"]="Invaild User name or password";
    header("location:con1.php");
 }
?>
<!DOCTYPE html>
<html lang="en">
